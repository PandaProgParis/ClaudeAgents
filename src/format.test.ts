...
import { abbreviateModel, contextLimitFor, formatDuration, formatRelativeTime, formatTokens, modelFamily } from './format';
...
describe('abbreviateModel', () => {
  it('réduit un id de modèle Claude à sa famille', () => {
    ...
    expect(abbreviateModel('claude-fable-5-1')).toBe('fable');
    expect(abbreviateModel('claude-mythos-5-1')).toBe('mythos');
    expect(abbreviateModel('claude-opus-5')).toBe('opus');
    expect(abbreviateModel('claude-opus-4-8')).toBe('opus');
    expect(abbreviateModel('claude-haiku-4-5-20251001')).toBe('haiku');
  });

  it('affiche tel quel un modèle hors des familles connues', () => {
    expect(abbreviateModel('claude-zeus-6')).toBe('claude-zeus-6');
    expect(abbreviateModel('gpt-x')).toBe('gpt-x');
  });
});

describe('modelFamily', () => {
  it('reconnaît les cinq familles Claude', () => {
  ...
  it('retourne undefined hors des familles connues', () => {
    expect(modelFamily('claude-zeus-6')).toBeUndefined();
    expect(modelFamily('claude-opusx-5')).toBeUndefined();
    expect(modelFamily('opus')).toBeUndefined();
  });
});
...
describe('contextLimitFor', () => {
  ...
  it('retourne undefined pour un modèle inconnu', () => {
    ...
    expect(contextLimitFor('claude-zeus-6')).toBeUndefined();
